...
const localizeHTML = {
  ...
  'tds.webview.path.smartclient': localize(
    'tds.webview.path.smartclient',
  ...
  'tds.webview.dir.include.info.or': localize(
    'tds.webview.dir.include.info.or',
    'or'
  ),
};
        ...
        (message) => {
          switch (message.command) {
            case 'checkDir':
              ...
              break;
            case 'welcomeClose':
              serverManager.setIncludePath(message.includes);
...
function getWebViewContent(context: vscode.ExtensionContext, localizeHTML) {
  const htmlOnDiskPath = vscode.Uri.file(
    path.join(context.extensionPath, 'src', 'welcome', 'welcomePage.html')
  );

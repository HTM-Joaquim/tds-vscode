...
const localizeHTML = {
  ...
  'tds.webview.ws.URL': localize(
    'tds.webview.ws.URL',
    'URL Web Service / WSDL FIle'
  ),
...
export default function showWSPage(context: vscode.ExtensionContext) {
  if (currentPanel) {
    ...
    let server: IServerDebugger = serverManager.currentServer;
    if (server) {
        ...
        (message) => {
          switch (message.command) {
            ...
            case 'wsClose':
              ...
              const extension: string = message.outputFileName
                .split('.')
                .pop()
                .toLowerCase();
              if (
                ...
                vscode.window.showErrorMessage(
                  'The output file must have one of the following extensions: .prw, .prx or .tlpp'
                );
                return;
              }
              if (_debugEvent) {
              ...
              server.generateWsdl(url).then(
                (response: any) => {
                  ...
                  if (fs.existsSync(pathFile)) {
                    vscode.window
                      .showWarningMessage(
                        'The file exists. Would like overwrite? ',
                        'Yes',
                        'No'
                      )
                      .then((clicked) => {
...
function createAndWriteOpen(filePath, content) {
  ...
  vscode.window
    .showInformationMessage(
      'The file was successfully created. Would like open? ',
      'Yes',
      'No'
    )
    .then((clicked) => {
...
function getWebViewContent(context: vscode.ExtensionContext, localizeHTML) {
  const htmlOnDiskPath = vscode.Uri.file(
    path.join(context.extensionPath, 'src', 'WebService', 'generateWS.html')
  );

...
const localizeHTML = {
  'tds.webview.inspect.generate': localize(
    'tds.webview.inspect.generate',
    'Patch Generation'
  ),
  'tds.webview.inspect.ignore.files': localize(
    'tds.webview.inspect.ignore.files',
    'Ignore files'
  ),
  'tds.webview.inspect.filter': localize(
    'tds.webview.inspect.filter',
    'Filter, ex: MAT or * All (slow)'
  ),
  'tds.webview.inspect.clean.selected': localize(
    'tds.webview.inspect.clean.selected',
    'Clear Selected'
  ),
  'tds.webview.inspect.clean.all': localize(
    'tds.webview.inspect.clean.all',
    'Clear All'
  ),
  'tds.webview.inspect.items': localize('tds.webview.inspect.items', 'Items'),
  'tds.webview.inspect.directory': localize(
    'tds.webview.inspect.directory',
    'Patch Generation Directory'
  ),
  'tds.webview.inspect.file.name.patch': localize(
    'tds.webview.inspect.file.name.patch',
    'Patch file name'
  ),
  'tds.webview.inspect.file.name': localize(
    'tds.webview.inspect.file.name',
    'File name'
  ),
  'tds.webview.inspect.items.generate': localize(
    'tds.webview.inspect.items.generate',
    'Generate'
  ),
  'tds.webview.inspect.items.generate.close': localize(
    'tds.webview.inspect.items.generate.close',
    'Generate/Close'
  ),
  'tds.webview.inspect.message1': localize(
    'tds.webview.inspect.message1',
    'The generated patch is based on the files from RPO. Be sure that the included fonts are compiled.'
  ),
  'tds.webview.inspect.items.showing': localize(
    'tds.webview.inspect.items.showing',
    'Items showing'
  ),
};
      ...
      (message) => {
        switch (message.command) {
          ...
          case 'exportData':
            ...
            let pathFile =
              vscode.workspace.rootPath + '/inspectorFunctions.txt';
            const textString = allItems.join('\n');

            if (fs.existsSync(pathFile)) {
              let r = Math.random().toString(36).substring(7);
              pathFile =
                vscode.workspace.rootPath + '/inspectorFunctions' + r + '.txt';
            }

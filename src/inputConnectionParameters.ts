...
import { EnvironmentTreeItem, ServerTreeItem } from './serverItemProvider';
import { IReconnectOptions } from '@totvs/tds-languageclient';
 ...
 */
export async function inputConnectionParameters(
  ...
  const addEnvironmentButton = new NewEnvironmentButton(
    {
      dark: Uri.file(
        path.join(__filename, '..', '..', 'resources', 'dark', 'add.png')
      ),
      light: Uri.file(
        path.join(__filename, '..', '..', 'resources', 'light', 'add.png')
      ),
    },
  ...
  async function collectConnectInputs() {
    ...
    if (serverParam instanceof ServerTreeItem) {
      ...
      await MultiStepInput.run((input) =>
        pickServer(input, state, serversConfig)
      );
    }
    ...
    if (reconnect) {
      ...
      let serverId =
        typeof state.server === 'string'
          ? state.server
          : (state.server as QuickPickItem).detail;
      let environmentName =
        typeof state.environment === 'string'
          ? state.environment
          : (state.environment as QuickPickItem).label;
      //@acandido
      // state.reconnectionToken = serverManager.getAuthorizationToken(undefined);
      //serverId
      //environmentName
    }
    ...
    if (environments.length > 0) {
      const pick = await input.showQuickPick({
        ...
        placeholder: localize(
          'tds.vscode.select_environment',
          'Select environment'
        ),
        items: environments,
        activeItem:
          typeof state.environment !== 'string' ? state.environment : undefined,
      ...
      if (pick instanceof NewEnvironmentButton) {
        return (input: MultiStepInput) =>
          inputEnvironment(input, state, serversConfig);
      }
  ...
  async function inputEnvironment(
    input: MultiStepInput,
    state: Partial<State>,
    serversConfig: any
  ) {
  ...
  async function getEnvironments(
    ...
    let target;
    if (state.server) {
      target = serversConfig.find((target: any) => {
        return (
          target.id ===
          (typeof state.server !== 'string'
            ? state.server.detail
    ...
    return (target ? target.environments : []).map((name) => ({ label: name }));
  }

  async function main() {
    const connectState = await collectConnectInputs();
    const server = serversConfig.find((target: any) => {
      return (
        target.id ===
        (typeof connectState.server !== 'string'
          ? connectState.server.detail
            ? connectState.server.detail
            : ''
          : connectState.server)
      );
    ...
    if (connectState.reconnectionToken) {
      let environmentName =
        typeof connectState.environment === 'string'
          ? connectState.environment
          : (connectState.environment as QuickPickItem).label;
      //@acandido
      const options: Partial<IReconnectOptions> = {
        connType: connType,
        environment: environmentName,
      };
      server.reconnect(options); //, connType);
    } else {
      const environment =
        typeof connectState.environment !== 'string'
          ? connectState.environment.toString()
          : connectState.environment;
      server.name = server.name; //FIX: quebra-galho necessário para a árvore de servidores
      //@acandido
      //server.connect(server, environment, connType);
      //   const options: Partial<IConnectOptions> = {
      //     connType: connType,
      //     environment: environmentName,
      //   };
      server.connect(environment);
    }
  }

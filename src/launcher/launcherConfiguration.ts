...
export default class LauncherConfiguration {
  static show(context: vscode.ExtensionContext) {
    if (currentPanel) {
      ...
      try {
        launchersInfo = TDSConfiguration.loadLaunchConfig()();
        ...
        switch (message.command) {
          case 'saveLaunchConfig':
            ...
            if (launchersInfo !== undefined) {
              ...
              TDSConfiguration.saveLaunchConfig(launchersInfo);
              currentLaunchersInfoContent = launchersInfo; //fs.readFileSync(TDSConfiguration.loadLaunchConfig()File(),'utf8');
              ...
              vscode.window.showInformationMessage(
                localize('tds.vscode.launcher.executor', 'Executor') +
                  ' ' +
...
function addLaunchJsonListener(): void {
  let launchJson = TDSConfiguration.loadLaunchConfig();
  ...
  if (fs.existsSync(launchJson)) {
    fs.watch(launchJson, { encoding: 'buffer' }, (eventType, filename) => {
      if (filename && eventType === 'change') {
        let tmpLaunchIfo = TDSConfiguration.loadLaunchConfig();
